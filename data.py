...
# ---------------------------------------------------------------------------
# HPE GreenLake: on-prem consumption (pay-per-use) pricing
# ---------------------------------------------------------------------------
# GreenLake is a THIRD cost mode, distinct from both "own on-prem (CapEx)" and
# "rent public cloud (EC2 OpEx)": the GPU hardware sits physically in your facility
# (so data and model weights can stay on-prem), but it is billed as metered OpEx with a
# committed baseline plus burst, and there is little or no upfront CapEx.
#
# WARNING: GreenLake pricing is negotiated and not public. The numbers below are
# PLACEHOLDERS, plausible for an L40S/H100-class GPU tier; replace them with an actual
# vendor quote before trusting any breakeven. Because the hardware sits in your
# datacenter, facility power + rack/cooling are still your cost unless the deal
# explicitly bundles them.
GREENLAKE_DEFAULTS = {
    "committed_baseline_monthly": 4000.0,   # PLACEHOLDER: reserved committed spend per month (take-or-pay)
    "committed_gpu_hours_month": 730.0,     # GPU-hours the committed baseline covers (~1 GPU * 730h)
    "burst_rate_gpu_hr": 3.50,              # PLACEHOLDER: metered $/GPU-hour above the committed baseline
    "install_fee": 0.0,                     # one-time onboarding/install; usually ~0 for GreenLake
    "facility_is_ours": True,               # power + rack billed to you (hardware in your DC)
}


def calc_greenlake_monthly(
    total_gpu_hours: float,
    committed_baseline_monthly: float = GREENLAKE_DEFAULTS["committed_baseline_monthly"],
    committed_gpu_hours_month: float = GREENLAKE_DEFAULTS["committed_gpu_hours_month"],
    burst_rate_gpu_hr: float = GREENLAKE_DEFAULTS["burst_rate_gpu_hr"],
    facility_opex_monthly: float = 0.0,
) -> float:
    """Monthly GreenLake cost: committed baseline + metered burst above it + your facility opex.

    total_gpu_hours is the GPU-hours actually consumed in the month (gpu_count * hours_run).
    The committed baseline is take-or-pay (paid whether or not fully used); burst is only
    charged for consumption above committed_gpu_hours_month. facility_opex_monthly is the
    on-prem power + rack cost, since the hardware sits in your datacenter.
    """
    burst_hours = max(0.0, total_gpu_hours - committed_gpu_hours_month)
    return committed_baseline_monthly + (burst_hours * burst_rate_gpu_hr) + facility_opex_monthly
