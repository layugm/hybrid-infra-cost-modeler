...
plain_summary = summary.replace("**", "")          # plain text for the download file
# Escape $ so Streamlit markdown does not treat "$...$" as LaTeX math (which collapses
# spaces and mangles the dollar figures). Keep the ** bold ** for the on-screen render.
display_summary = summary.replace("$", "\\$")

st.container(border=True).markdown(display_summary)
